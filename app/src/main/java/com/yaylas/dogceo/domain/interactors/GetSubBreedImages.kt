...
class GetSubBreedImages @Inject constructor(
    private val connectionManager: ConnectionManager,
    private val localDataSource: LocalDataSource,
    private val networkDataSource: NetworkDataSource
) {

    suspend fun execute(breed: String, parentBreed: String): Flow<DataState<List<ImageInfo>>> = flow {
        emit(DataState.Loading)
        try {
            if (connectionManager.isConnected()) {
                val networkImages = networkDataSource.getSubBreedImages(breed, parentBreed)
                localDataSource.insertImageList(networkImages)
            }
            val data = localDataSource.getBreedImages(breed, parentBreed)
